...
def dashboard_callback(request, context):
    ...
    can_users = request.user.is_superuser or request.user.has_perm("accounts.view_user")
    forms = Form.objects.filter(deleted_at__isnull=True).aggregate(
        total=Count("id"), published=Count("id", filter=Q(status="PUBLISHED")),
    ) if can_forms else {}
    responses = Submission.objects.aggregate(
        total=Count("id"),
    ...
    if request.user.is_superuser and isinstance(default_storage, FileSystemStorage):
        try:
            ...
            storage_percent = round((disk.total - disk.free) / disk.total * 100) if disk.total else 0
        except OSError:
            pass
    context.update({
        "dashboard_cards": [
            {"label": "Respuestas recibidas", "value": responses.get("total", "—"),
             "note": f"{responses.get('recent', 0)} en los últimos 7 días"
             if can_responses else "Sin permiso para consultar respuestas",
             "icon": "inbox", "color": "purple"},
            {"label": "Formularios publicados", "value": forms.get("published", "—"),
             "note": f"De {forms.get('total', 0)} formularios en total"
             if can_forms else "Sin permiso para consultar formularios",
             "icon": "description", "color": "blue"},
            {"label": "Usuarios registrados", "value": users.get("total", "—"),
             "note": f"{users.get('active', 0)} cuentas activas en la plataforma"
             if can_users else "Solo visible para administradores de usuarios",
             "icon": "group", "color": "green"},
            {"label": storage_title, "value": storage_value, "note": storage_note,
             "icon": "cloud", "color": "orange", "percent": storage_percent},
        ],
        "dashboard_forms": list(Form.objects.filter(deleted_at__isnull=True).annotate(response_count=Count("submissions"))
                                .order_by("-updated_at")[:6]) if can_forms and can_responses else [],
