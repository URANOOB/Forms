...
def theme_context():
    return {
        ...
        "theme_fonts": [(key, value[0]) for key, value in FONTS.items()],
        "theme_colors": COLORS,
        "theme_background_colors": BACKGROUND_COLORS,
        "theme_config": {
            "defaults": DEFAULTS,
            "fonts": {key: value[1] for key, value in FONTS.items()},
            "image_url_template": reverse("form_image", args=[uuid.UUID(int=0)]).replace(
    ...
    if result["header_image"]:
        try:
            ...
            raise ValidationError(
                "La imagen de encabezado no pertenece a este formulario."
            ) from None
